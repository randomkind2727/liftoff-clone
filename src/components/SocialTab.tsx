  ...
  return (
    <div className="px-5 pt-2 space-y-5 tab-enter">
      ...
      <div className="glass-strong rounded-2xl p-4 flex items-center gap-4">
        <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center text-2xl glow-purple">
          🦉
        </div>
        ...
        <div className="text-right">
          <p className="font-bold text-yellow-400">{totalXP} XP</p>
          ...
          {leaderboard.map((f, i) => (
            <motion.div
              ...
              className={`flex items-center gap-3 p-2.5 rounded-xl ${
                f.id === 'me' ? 'bg-purple-500/10 border border-purple-500/20' : ''
              }`}
            >
              <div className="w-7 text-center">
                {i < 3 ? <span className="text-lg">{medals[i]}</span> : <span className="text-xs text-slate-500">#{i+1}</span>}
              </div>
